...
behaviors.sections = function() {
	...
	$('article section').each(function() {
		...
		if (toc) {
			var selector = $('<li class="entry selector"><a href="#' + id + '">' + title + '</a></li>').appendTo(toc);
...
function createCode(element) {
	...
	if (highlight) {
		...
		for (var i = 0, l = highlights.length; i < l; i++) {
			...
			var hlEnd = highlight.length == 2 ? parseInt(highlight[1], 10) - 1 : hlStart;
			if (start) {
				hlStart -= start - 1;
				hlEnd -= start - 1;
			}
...
function toggleMember(member, dontScroll, offsetElement) {
	...
	if (!dontScroll) {
		...
		$(document).scrollTop(scroll + (visible ? desc : link).offset().top - offset + 11 * (visible ? 1 : -1));
	}
